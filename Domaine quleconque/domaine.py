...
if (m==2):
    ...
    l=[]
    for i in range(m):
        l.append([int((i+1/2)*(n/m)),random.randint(1,int((n/(2*m)))-2),random.randint(1,c+1)])
    for elt in l:
...
plt.contourf(Z, Y, z, cmap=cm.jet)  # You can also use plt.pcolormesh for a different style
